...
namespace Hu.MachineVision.VisionPro
{
    class StationViewCompValue: IDataViewStation
    {
        ...
        public TabPage Tp { get { return MyTabs["CompValue", CcdId]; } }
        ...
        public static StationViewCompValue[] Stations { get; set; } 

        static StationViewCompValue()
        {
            Dgvs = new DataGridView[CcdCount];
            EditStations = new StationToolBlockEdit[CcdCount];
            Panels = new Dictionary<string, Panel>[CcdCount];

            Stations = new StationViewCompValue[CcdCount];
            
            for(int i = 0; i < CcdCount; i++)
            {
                var tp = MyTabs["CompValue", i];                
            ...
            for (int i = 0; i < CcdCount; i++)
            {
                Stations[i] = new StationViewCompValue(i);
            }
        }

        public StationViewCompValue(int ccdId)
        {
            CcdId = ccdId;
        }
